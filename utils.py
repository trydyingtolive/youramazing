...
def upscale(maze, line,path):
    """Upscales maze."""
    if not isinstance(maze, np.ndarray):
        maze = np.array(maze)
    if path <= 1:
        return maze
    s = np.array([line-3,path+3]*((len(maze)-1)//2))
    s = np.append(s, [line])
    maze = maze.repeat(s,  axis=0)
    
    s= np.array([line,path]*((len(maze[0])-1)//2))
    s= np.append(s, [line])
    
    return maze.repeat( s, axis=1)
